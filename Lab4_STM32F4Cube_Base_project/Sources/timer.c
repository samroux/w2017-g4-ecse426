...
void Timer_Init(void)
{
  ...
  sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
	...
	HAL_TIM_PWM_ConfigChannel(&tim4_handle, &sConfigOC, TIM_CHANNEL_1); //GREEN

  HAL_TIM_PWM_ConfigChannel(&tim4_handle, &sConfigOC, TIM_CHANNEL_2); //ORANGE

  HAL_TIM_PWM_ConfigChannel(&tim4_handle, &sConfigOC, TIM_CHANNEL_3);	//RED

  HAL_TIM_PWM_ConfigChannel(&tim4_handle, &sConfigOC, TIM_CHANNEL_4);	//BLUE
...
void LEDs_Init(void){
	...
	B_GPIO.Pin = GPIO_PIN_6;	//TIM4_CH1 -> Green/Red
  ...
  B_GPIO.Speed = GPIO_SPEED_FREQ_HIGH;
	...
	D_GPIO.Pin = GPIO_PIN_13;	//Orange
	HAL_GPIO_Init(GPIOD, &D_GPIO);
	
	D_GPIO.Pin = GPIO_PIN_14;	//Red
	HAL_GPIO_Init(GPIOD, &D_GPIO);
	
	D_GPIO.Pin = GPIO_PIN_15;	//Blue
	HAL_GPIO_Init(GPIOD, &D_GPIO);
	
	B_GPIO.Pin = GPIO_PIN_7;			//TIM4_CH2	-> Orange/Blue
	HAL_GPIO_Init(GPIOB, &B_GPIO);
}
